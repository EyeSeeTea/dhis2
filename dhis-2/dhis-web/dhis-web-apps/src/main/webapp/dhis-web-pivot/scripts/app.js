  ...
  Ext.define('Ext.ux.button.ColorButton', {
	...
	LegendSetWindow = function() {
		...
		legendSetStore = Ext.create('Ext.data.Store', {
			...
			proxy: {
				type: 'ajax',
				url: ns.core.init.contextPath + '/api/legendSets.json?fields=id,displayName|rename(name)&paging=false',
				reader: {
					type: 'json',
					root: 'legendSets'
				},
		...
		LegendSetPanel = function() {
			...
			legendSetGrid = Ext.create('Ext.grid.Panel', {
				...
				listeners: {
					render: function() {
						var that = this,
							maxHeight = ns.app.viewport.centerRegion.getHeight() - 155,
							height;

						this.store.on('load', function() {
							if (Ext.isDefined(that.setHeight)) {
								height = 1 + that.store.getCount() * 27;
		...
		LegendPanel = function(id) {
            ...
            LegendEditWindow = function(record) {
                ...
                editLegendName = Ext.create('Ext.form.field.Text', {
                    ...
                    fieldLabel: NS.i18n.legend_name,
                ...
                editWindow = Ext.create('Ext.window.Window', {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            items: [
                                {
                                    html: NS.i18n.legend_symbolizer + ':',
                                    width: fieldLabelWidth,
                                    bodyStyle: 'background:transparent; padding-top:3px; padding-left:3px'
                                },
                                editColor
                        ...
                        },
                        {
                            ...
                            items: [
                                {
                                    html: NS.i18n.legend_symbolizer_background + ':',
                                    width: fieldLabelWidth,
                                    bodyStyle: 'background:transparent; padding-top:3px; padding-left:3px'
                                },
                                editBgColor
			...
			legendSetName = Ext.create('Ext.form.field.Text', {
                ...
                labelStyle: 'padding-top: 5px; padding-left: 3px',
				fieldLabel: NS.i18n.legend_set_name,
                style: 'margin-bottom: 6px'
			});
			...
			panel = Ext.create('Ext.panel.Panel', {
				...
				items: [
					...
					{
                        xtype: 'container',
						html: NS.i18n.add_legend,
						cls: 'ns-panel-html-title',
			...
			if (id) {
				legendStore.proxy.url = ns.core.init.contextPath + '/api/legendSets/' + id + '.json?fields=legends[id,displayName|rename(name),startValue,endValue,color,bgColor]';
				legendStore.load();
		...
		storeLegendSets = Ext.create('Ext.data.Store', {
			fields: ['id', 'name'],
			proxy: {
				type: 'ajax',
				url: ns.core.init.contextPath + '/api/legendSets.json?fields=id,displayName|rename(name)&paging=false',
		...
		deleteLegendSet = function(id) {
			if (id) {
				Ext.Ajax.request({
					url: ns.core.init.contextPath + '/api/legendSets/' + id,
					method: 'DELETE',
					success: function() {
						legendSetStore.load();
						storeLegendSets.load();
					}
				});
			}
		};
		...
		create = Ext.create('Ext.button.Button', {
			...
			handler: function() {
				if (legendSetName.getValue() && validateLegends()) {
					...
					Ext.Ajax.request({
						url: ns.core.init.contextPath + '/api/legendSets/',
						method: 'POST',
						headers: {'Content-Type': 'application/json'},
						params: body,
						success: function() {
		...
		update = Ext.create('Ext.button.Button', {
			...
			handler: function() {
				if (legendSetName.getValue() && validateLegends()) {
					...
					Ext.Ajax.request({
						url: ns.core.init.contextPath + '/api/legendSets/' + id + '?mergeStrategy=REPLACE',
						method: 'PUT',
						headers: {'Content-Type': 'application/json'},
						params: body,
						success: function() {
		...
		window = Ext.create('Ext.window.Window', {
			title: NS.i18n.legendsets,
			iconCls: 'ns-window-title-icon-legendset',
            bodyStyle: 'padding:1px; background-color:#fff',
			resizable: false,
			width: windowWidth,
			modal: true,
...
console.log(table);
	...
	createViewport = function() {
		...
		legendsButton = Ext.create('Ext.button.Button', {
			text: NS.i18n.legends,
			menu: {},
			handler: function() {
				if (ns.app.legendSetWindow) {
					ns.app.legendSetWindow.destroy();
					ns.app.legendSetWindow = null;
				}

				ns.app.legendSetWindow = LegendSetWindow();
				ns.app.legendSetWindow.show();
			},
			listeners: {
				added: function() {
					ns.app.legendsButton = this;
				}
			}
		});
		...
		centerRegion = Ext.create('Ext.panel.Panel', {
			...
			tbar: {
				...
				items: Ext.Array.clean([
					...
					favoriteButton,
					ns.core.init.user.isAdmin ? legendsButton : null,
					downloadButton,
					shareButton,
					'->',
					defaultButton,
                            ...
                            success: function(r) {
                                ...
                                Ext.Ajax.request({
                                    ...
                                    success: function(r) {
                                        ...
                                        // legend sets
                                        requests.push({
                                            url: contextPath + '/api/legendSets.json?fields=id,displayName|rename(name),legends[id,displayName|rename(name),startValue,endValue,color,bgColor]&paging=false',
                                            success: function(r) {
